...
| `USER.PROFILE` | GET `nuke.php?__lib=ucp&__act=get&lite=js&noprefix&uid|username`; profile Referer; username GBK encoded. | Removes JS/comments and repairs numeric tokens; parses profile; raw parse failure logged; active avatar URL updated locally. | `nga_phone_base_3.0/.../task/JsonProfileLoadTask.java:47-114`; `.../activity/ProfileActivity.java:125-151,511-515` | `modified`: the profile reader shares envelope normalization and omits its local raw parse-failure log. Delivered online thread pages also request latest public `ipLoc` through a dedicated bounded transport with explicit identity headers, account cache, one call in flight, and an app-wide one-second pause after each call completes. See [the author-location contract](./author-profile-location-contract.md). |
