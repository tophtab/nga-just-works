...
| `USER.PROFILE` | GET `nuke.php?__lib=ucp&__act=get&lite=js&noprefix&uid|username`; profile Referer; username GBK encoded. | Removes JS/comments and repairs numeric tokens; parses profile; raw parse failure logged; active avatar URL updated locally. | `nga_phone_base_3.0/.../task/JsonProfileLoadTask.java:47-114`; `.../activity/ProfileActivity.java:125-151,511-515` | `modified`: the profile reader shares envelope normalization and omits its local raw parse-failure log. Automatic thread-author `ipLoc` queries are disabled on main, including offscreen prefetch; the retained helpers have no runtime entry. Active automatic-query experiments use `experiment/auto-ip-query`. See [the author-location contract](./author-profile-location-contract.md). |
