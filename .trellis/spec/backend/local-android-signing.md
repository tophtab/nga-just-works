...
Use this contract when the maintainer requests a local signed APK or an update
that preserves the installed application's data. The authorization and Windows
ADB rules in [Android Quality and Instrumentation](./android-quality-guidelines.md)
still apply; discovering a key does not authorize building or operating a device.

## 2. Signatures

The maintainer's WSL signing configuration was verified on 2026-09-06:
...
The maintainer also stores the same keystore and credential file in
[tophtab/nga-just-works-signing](https://github.com/tophtab/nga-just-works-signing),
a separate private repository. Its visibility, a fresh remote clone, file
equality, and the cloned private-key entry were verified on 2026-09-06.
...
- Load the credentials privately into the build process environment. Do not
  ...
  arguments, or copy signing material into the public application's worktree.
...
- Do not commit the keystore, `credentials.env`, or a base64 copy to this
  public repository, Releases, or ordinary Actions artifacts. Record only
  paths, environment-variable names, and public certificate metadata.
- The separately authorized private signing repository may contain both
  original signing files. Verify its owner and private visibility before an
  upload. The application's CI continues to use its existing GitHub Actions
  signing Secrets.
...
| The configured store path does not exist | Report the local configuration error before starting the build |
...
| The signing repository is not confirmed private and owned by `tophtab` | Do not upload signing material |

## 5. Good / Base / Bad Cases

- **Good:** the local key opens successfully, its certificate matches the
  installed app, and the signed preview updates that app while retaining data.
- **Base:** the local certificate is verified, but a build or device operation
  has not been requested. Stop after the authorized checks.
- **Bad:** assume the key is unavailable after checking only environment
  variables, or install the ordinary `.debug` variant expecting an in-place update.

## 6. Tests Required

- When verifying local signing material, validate the store password
  and private-key entry with `keytool`, then compare the public certificate.
- After uploading to the private signing repository, clone the remote into a
  separate directory, compare both files with the originals, and verify the
  cloned keystore's private-key entry and public certificate.
...
```bash
# Signing material must stay outside the public application repository.
